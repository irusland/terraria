...
namespace Digger
{
    public class GameState
    {
        ...
        {
            var creaturesPerLocation = GetCandidatesPerLocation(game);
            for (var x = 0; x < game.MapWidth; x++)
            for (var y = 0; y < game.MapHeight; y++)
                    game.Map[x, y] = SelectWinnerCandidatePerLocation(creaturesPerLocation, x, y, game);
        }

        private static ICreature SelectWinnerCandidatePerLocation(List<ICreature>[,] creatures, int x, int y, Game game)
        {
            var candidates = creatures[x, y];
            var aliveCandidates = candidates.ToList();
            foreach (var candidate in candidates)
            foreach (var rival in candidates)
                if (rival != candidate && candidate.DeadInConflict(rival, game))
        ...
        private List<ICreature>[,] GetCandidatesPerLocation(Game game)
        {
